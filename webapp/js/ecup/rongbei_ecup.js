...
if (currUser.length==0) {
    window.location.href = "http://m.ruwe.cn/wx/auth/bridge?url=http%3a%2f%2fm.ruwe.cn%2fhtml%2fecup%2frongbei_ecup.html";
}
$(document).ready(function() {
    ...
    //获取点击数
    $.ajax({
        url:"/api/web/lottery/clicks/100004-100004-100039-9",
        type: "get",
        dataType:"json",
    ...
    var uid = localStorage.getItem('rbbankuid');
    if(!uid){
        uid = Uid();
        localStorage.setItem('rbbankuid',uid);
    }
    //奖品显示
    ...
    var prize500 = '<div class="zhichong"><img class="prize_name" src="../../images/ecup/500M.png"/>'+
        '<p class="cztext">正在为您充值<br/>充值成功会以短信的形式发送到您的手机</p></div>';
    var prize30 = '<div class="zhichong"><img class="prize_name" src="../../images/ecup/30M.png"/>'+
        '<p class="cztext">正在为您充值<br/>充值成功会以短信的形式发送到您的手机</p></div>';
    ...
    $(".golottery").click(function () {
        ...
        if (phonenum.match(/^(13|15|18|14|17)\d{9}$/) && phonenum != '') {
            $.ajax({
                url:"/lottery/check/100004-100004-100039",
                type: "get",
                dataType:"json",
                data:{
    ...
    $(".page").click(function(){
        ...
        var prize500m = '<div class="zhichong"><img class="prize_name" src="../../images/ecup/500M.png"/>'+
            '<p class="cztext">正在为您充值<br/>充值成功会以短信的形式发送到您的手机</p></div>';
        var prize30m = '<div class="zhichong"><img class="prize_name" src="../../images/ecup/30M.png"/>'+
            '<p class="cztext">正在为您充值<br/>充值成功会以短信的形式发送到您的手机</p></div>';
        ...
        $.ajax({
            url:"/lottery/100004-100004-100039",
            type: "post",
            dataType:"json",
            data:{
                index:index,
            ...
            success: function (result) {
                if(result.status == 0) {
                    ...
                    if (prizeSize == "30") {
                        card1.find(".pagebg").html(prize30m);
                    } else if (prizeSize == "500") {
